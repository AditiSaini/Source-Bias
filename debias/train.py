                ...
                for corpus_id, score in qrels[query_id].items():
                    if score >= 1: # if score = 0, we don't consider for training
                        try:
                            ...
                            print("Corpus ID - ", type(corpus_id))
                            human_doc = human_corpus[str(corpus_id)].get("text")
                            llm_doc = llm_corpus[str(corpus_id)].get("text")
                            train_samples.append(InputExample(guid=idx, texts=[query, human_doc, llm_doc], label=1))
                        except KeyError as e:
                            logging.error("Error: Key {} not present in corpus! - {}".format(corpus_id, e))
